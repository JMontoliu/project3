resource "google_artifact_registry_repository" "repo" {
  ...
  location      = var.region
  repository_id = var.repository_name4
  format        = "DOCKER"
}
...
locals {
  image_url = "${var.region}-docker.pkg.dev/${var.project_id}/${var.repository_name4}/${var.image_name4}:latest"
}
...
resource "google_cloud_run_v2_service" "streamlit" {
  name                = var.cloud_run_service_name4
  location            = var.region
  project             = var.project_id
  ...
  template {
    containers {
      image = "europe-west1-docker.pkg.dev/${var.project_id}/${var.repository_name4}/${var.image_name4}:latest"
